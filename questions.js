...
function loadInputQuestion(randomQuestion, optionsElement, index) {
  ...
  // Split the question text by "______"
  let parts = randomQuestion.question.split('______')
  ...
  parts.forEach((part, idx) => {
    ...
    if (idx < randomQuestion.input.length) {
      let inputField = randomQuestion.input[idx]

      let inputElement = document.createElement('input')
      inputElement.type = 'text'
      inputElement.className = 'answer-input'
      inputElement.placeholder = inputField.operand || 'Answer'
